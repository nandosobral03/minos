...
type Node = {
  ...
  valid: boolean;
    ...
    },
    {
      ...
      explanation: `The neighbors of the currentNode are found. If the neighbor has not been visited and is valid, the distance from the starting node is calculated. If the distance is less than the neighbor's distance, the neighbor's distance is set to the distance and the neighbor's previous node is set to the currentNode.`,
    },
    {
      text: `
    }
    return [];
      ...
      explanation: `The loop ends and an empty array is returned because there are no more nodes to visit which means that there is no path.`,
    },
    {
      text: `
const getClosestNode = (nodes: Node[]): Node => {
  return nodes.reduce((closest, node) => (node.distance < closest.distance ? node : closest));
}`,
      explanation: `The getClosestNode function takes an array of nodes and returns the node with the smallest distance.`,
    },
    {
      text: `
const getNeighbors = (nodes: Node[][], node: Node): Node[] => {
  ...
  return neighbors;
      ...
      explanation: `The getNeighbors function takes a 2D array of nodes and a node and returns an array of the node's neighbors.`,
    },
    {
      text: `
const getPath = (endNode: Node): [number,number][] => {
  ...
  return path;
      ...
      explanation: `The getPath function takes an end node and returns the path from the starting node to the end node, using the previous property of each node, which is set in the main function.`,
    }
